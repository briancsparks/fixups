...
s.fns.lookupByTag = function(instanceData, tag, value) {
  const tag_    = idify(tag);
  const value_  = idify(value);
  const key     = `tag_${tag_}_${value_}`;

  return instanceData.by[key];
};

s.fns.instanceIds = function(arr) {
  return arr.map((inst) => inst.InstanceId).filter((x) => x);
};

/**
 *  Converts the standard AWS Reservations struct into a list of augmented
 *  instance objects, and an index.
 */
